...
> [!NOTE]
> **This fork** ([CharlesABear/slack-mcp-server](https://github.com/CharlesABear/slack-mcp-server)) adds **Slack Canvas tools** and is published to npm as **`@charlesabear/slack-mcp-server`**. Install it with `npx -y @charlesabear/slack-mcp-server@latest` (see [Using npx](docs/03-configuration-and-usage.md#using-npx)). The upstream `slack-mcp-server` package does **not** include the canvas tools. Maintainers of their own fork: see [Publishing the fork to npm](docs/03-configuration-and-usage.md#publishing-the-fork-to-npm).
